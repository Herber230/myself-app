 ...
 * `commitlint.config.mjs` (which runs from `.husky/commit-msg`, before anything
 * is built), by `check-pull-request.mjs` in CI *and* by `conventions.spec.ts`.
 * That is what keeps the rule a developer's commit is judged by and the rule CI
 * asserts from ever being two different rules.
 *
 ...
 *
 * Commits, pull requests and documents in this repository carry no AI or tool
 * attribution — the same convention as entifix and r10c. There it was stated in
 * prose first and skipped anyway, on 5 of 40 merged pull requests and 6 of 60
 * commits, because an agent instruction arriving every session claimed to
 * replace earlier guidance. A sentence cannot win that argument; a failing check
 * can. So this repository starts with the check, copied from entifix.
 */
 ...
 * "Claude" alone: this repository legitimately mentions the tool — `CLAUDE.md`,
 * the `.claude/` entry in `.gitignore` — and a matcher broad enough to catch
...
/*
 * ⚠️ `.claude/` is **deliberately absent** from that list. It is gitignored, the
 * repository scan reads `git ls-files`, and an untracked file can never reach
 * it — so an exemption for it would do nothing except fail wherever the
 * directory is not checked out, which is how the same entry failed entifix's
 * first CI run. An exemption is only meaningful for a **tracked** path.
 */
