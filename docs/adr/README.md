...
- [0009](0009-a-radar-laid-out-at-build-time.md) — **The radar's maths is ported, and it runs at build time.** Read when the radar's blips moved between two builds, or a radar library is proposed.
- [0010](0010-content-checked-from-entity-metadata.md) — **Content is checked from entity metadata, and every package ships `dist`.** Read when a record fails `next build`, a member should be translated, a filter over a list matches nothing, or a package's code reaches the app as source.
- [0011](0011-a-moving-hero-a-revealing-nav-and-a-blue-theme.md) — **A moving hero, a revealing nav and a blue theme.** Read when the hero's motion or the nav's reveal misbehaves with scripting off or reduced motion, a first visit paints a theme you did not expect, or a printed CV comes out dark.
- [0012](0012-the-cv-as-a-paper-sheet.md) — **The CV as a paper sheet, in variants and two modes.** Read when a CV page spills onto a second page, an applicant tracking system reads the PDF wrong, a variant shows the wrong bullets, or a CV URL 404s under `serve-out`.
- [0013](0013-cloudfront-over-a-private-bucket-defined-in-pulumi.md) — **CloudFront over a private bucket, defined in Pulumi, released on every merge.** Read when a URL answers differently on `herbercolop.dev` than under `serve-out`, a deploy shipped stale HTML, a merge to `main` did not deploy, or an infrastructure change is being made by hand in the console.
- [0014](0014-a-personal-radar.md) — **A personal radar: rings for one person, editions in content, filtering over placed blips.** Read when a blip shows the wrong movement, a ring's meaning is questioned, a technology needs its own page, or the radar's filter is about to query in the browser.
- [0015](0015-customizing-the-cv-in-the-browser.md) — **Customizing the CV in the browser: the URL holds what is hidden, a stylesheet hides it.** Read when a customized CV flashes its hidden parts on load, loses a choice on reload, in print or after switching reading, or the customizer is about to query content in the browser.
